...
ncbi.search = function (db, term, cb) {
  ...
  var requestIDs = requestStream(true)
  var requestData = requestStream(true)
  ...
  requestIDs.on('error', function (err) {
    stream.write(URL.parse(err.config.uri, true).query.term)
  })
  requestData.on('error', function (err) {
    stream.write(err.obj.streamMetadata.term)
  })
...
function createAPIDataURL (opts) {
  ...
  function transform (obj, enc, next) {
    var totalCount = parseInt(obj.esearchresult.count, 10)
    if (totalCount === 0) { return next() }
    var esearchURL = URL.parse(obj.reqURL, true)
    var count = opts.limit || totalCount
    ...
    for (var i = 0; i < numRequests; i++) {
      ...
      // var req = {prevURL: obj.url, opts: opts, url: query }
      var req = {
        url: query,
        streamMetadata: {
          db: esearchURL.query.db,
...
function createLinkObj () {
  ...
  function transform (obj, enc, next) {
    var self = this
    xml2js(obj, gotParsed)
    function gotParsed (err, data) {
      if (err) { self.emit('error', err); return next() }
      var result = {
        srcDB: data.eLinkResult.LinkSet[0].DbFrom[0],
        destDB: data.eLinkResult.LinkSet[0].LinkSetDb[0].DbTo[0],
        srcUID: data.eLinkResult.LinkSet[0].IdList[0].Id[0]
      }
      if (data.eLinkResult.LinkSet[0].LinkSetDb) {
        data.eLinkResult.LinkSet[0].LinkSetDb.forEach(getMatch)
      }
      function getMatch (link) {
        var linkName = result.srcDB + '_' + result.destDB
        if (link.LinkName[0] !== linkName) { return }
        result.destUIDs = []
        link.Link.forEach(function (link) {
          result.destUIDs.push(link.Id[0])
        })
      }
      self.push(result)
      next()
...
function requestStream (returnURL) {
  return through.obj(function (obj, enc, next) {
    ...
    request(config, function gotData (err, res, body) {
      if (err
        || !res
        || res.statusCode !== 200
        || !body
        ...
        self.emit('error', {obj: obj, config: config, err: err, res: res, body: body})
        return setTimeout(next, 5000)
      }
      if (returnURL) { body.reqURL = url }
      if (obj.streamMetadata) {
        body.streamMetadata = obj.streamMetadata
      }
      self.push(body)
