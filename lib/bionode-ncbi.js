...
function createAPIDataURL(limit) {
  var counter = 0
  var max = limit < RETURNMAX ? limit : RETURNMAX
  var stream = through.obj(transform)
  return stream

  function transform(obj, enc, next) {
    if (counter === limit) { return this.push(null) }
    var count = limit || obj.esearchresult.count
    var numRequests = Math.ceil(count / max)
